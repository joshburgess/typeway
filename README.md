...
**Compile time discipline.** Type-level web frameworks are a stress test for any type checker. Servant had a reputation for slow compile times in its earlier years, but modern Servant has improved in this area. Typeway is engineered around the same goal from day one: flat tuple impls generated by `macro_rules!` replace recursive trait chains, method-indexed routing avoids O(routes) type-level dispatch, and type erasure at the router boundary (`BoundHandler`) prevents monomorphization blowup. The result is compile times that scale linearly with API surface, even on large services.
