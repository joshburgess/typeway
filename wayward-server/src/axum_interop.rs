//! Axum interoperability — embed a wayward API inside an Axum app.
//!
//! Enabled with `feature = "axum-interop"`. Provides conversions from
//! wayward's [`Server`] and [`Router`] into Axum types.
//!
//! # Example
//!
//! ```ignore
...
//!
//! let wayward_api = Server::<MyAPI>::new(handlers);
...
/// Adapter that bridges Axum's body type to wayward's router.
///
...
/// wayward router's handlers directly.
#[derive(Clone)]
struct AxumAdapter {
    router: Arc<Router>,
}
